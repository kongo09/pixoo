...
class Pixoo:
    ...
    def get_settings(self):
        response = requests.post(
            self.__url, json.dumps({"Command": "Channel/GetAllConf"}), timeout=10
        )
        data = response.json()
    ...
    def __load_counter(self):
        ...
        response = requests.post(
            self.__url, '{"Command": "Draw/GetHttpGifId"}', timeout=10
        )
        data = response.json()
        if data["error_code"] != 0:
            self.__error(data)
    ...
    def __reset_counter(self):
        ...
        response = requests.post(
            self.__url, json.dumps({"Command": "Draw/ResetHttpGifId"}), timeout=10
        )
        data = response.json()
        if data["error_code"] != 0:
            self.__error(data)
